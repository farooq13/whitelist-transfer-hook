...
impl<'info> WhitelistOperation<'info> {
    ...
    pub fn realloc_whitelist(&self, is_adding: bool) -> Result<()> {
        ...
        if is_adding { // Adding to whitelist
            ...
            // Calcualte rent required fro the new account size
            let lamports_required = (Rent::get()?).minimum_balance(new_account_size);
            // Determine additional rent required
            let rend_diff = lamports_required - account_info.lamports();
            ...
            system_program::transfer(cpi_context, rend_diff)?;
            ...
            let lamports_required = (Rent::get()?).minimum_balance(new_account_size);
            // Determine additional rent to be refund
