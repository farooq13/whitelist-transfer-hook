...
use anchor_spl::{
    token_2022::spl_token_2022::{
        extension::{
            transfer_hook::TransferHookAccount, BaseStateWithExtensions, BaseStateWithExtensionsMut, PodStateWithExtensionsMut
        },
        pod::PodAccount
    },
    token_interface::{
...
pub struct TransferHook<'info> {
    ...
    /// CHECK: source token account owner, can be SystemAccount or PDA by another program
...
impl<'info> TransferHook<'info> {
    ...
    pub fn check_is_transfer_hook(&mut self) -> Result<()> {
        ...
        // Unpack the account data as a PodStateWithExtensionMut
        // This will allow us to access the extension of the token
        ...
        // The returning struct has a `transferring` field that indicates if the account is in middle of a transfer operation
