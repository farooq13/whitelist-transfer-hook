...
describe("whitelist-transfer-hook", () => {
  ...
  it("Initializes the Whitelist", async () => {
    const tx = await (program.methods as any).initializeWhitelist()
      .accountsPartial({
  ...
  it("Add user to whitelist", async () => {
    const tx = await (program.methods as any).addToWhitelist(provider.publicKey)
      .accountsPartial({
        admin: provider.publicKey,
        whitelist,
      })
  ...
  it("Remove user to whitelist", async () => {
    const tx = await (program.methods as any).removeFromWhitelist(provider.publicKey)
      .accountsPartial({
        admin: provider.publicKey,
        whitelist,
      })
  ...
  it('Create ExtraAccountMetaList Account', async () => {
    const initializeExtraAccountMetaListInstruction = await (program.methods as any)
